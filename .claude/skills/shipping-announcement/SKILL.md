...
BUG_LOOKBACK_DAYS        = 1                    # FETCH BOUND ONLY (this is an hourly job): query tickets completed in the last 1 day; the `last_bug_completed_at` watermark is the HARD inclusion floor (see Source B step 2). NOT the inclusion rule.
```
...
- Else query Salesforce (CData `queryData`, fully-qualified table):
  ...
  - **`[Type]` gate — ACTIVE customers only (hard rule):** announce ONLY when the matched account's `Type` = `Customer - Active`. **Skip every other status silently**, including **`Customer - Onboarding`** (still ramping — the CSM does NOT need ship/fix announcements yet; skip **even if a CSM is assigned**), plus `Prospect`, churned, test, etc. Note skipped ones in the summary as "skipped: not Customer-Active (<type>)". This gate is independent of the CSM check — a non-active account is skipped regardless of whether it has a CSM.
  - **Also use `[Type]` to disambiguate** when a name returns **multiple** accounts: pick the single `Customer - Active` one (e.g. two "Posh" — one `Prospect`, one `Customer - Active` → the active one). If none is `Customer - Active`, skip. If multiple are `Customer - Active` and still ambiguous, treat as **needs-attention** — do not guess.
